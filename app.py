...
def run_lines():
    ...
    breakdown = get_roster_breakdown(get_league(headers,team_id))

    # get roster
    roster = get_roster(headers, team_id)

    roster_list = build_lineup(roster, breakdown)
...
def get_league(headers, team_id):
    url = "https://fantasysports.yahooapis.com/fantasy/v2/league/%s/settings" % team_id.split(".t")[0]
    r = requests.get(url, headers=headers)
...
def get_roster_breakdown(league_xml):
    ...
    for position in league.find_all("roster_position"):
        abbr = position.find("position").text
        count = position.find("count").text
        league_dict[abbr] = int(count)
...
def build_lineup(raw_roster, breakdown):
    roster_list = []

    for date, active_teams in nhl_schedule_map.items():
        used_positions = breakdown.copy()
        players = get_roster_rank(raw_roster)
        daily_lineup = []

        for player in players:
            # ignore IR players because we don't want to move them
            # and players that are not playing
            if ("IR" in player.get("positions") or nhl_to_yahoo_abbr.get(player.get("team_key")) \
                    not in active_teams) and len(player.get("positions")) <=2:
                continue

            natural_position = [i for i in player.get("positions") if i != "Util"][0]
            if used_positions.get(natural_position) > 0:
                daily_lineup.append(player.get("player_key"))
                used_positions[natural_position] = used_positions.get(natural_position) -1
                player['positions'] = natural_position

        # make lined up player rank mapping
        rank_map = {}
        for active_player in daily_lineup:
            for player in players:
                if player.get("player_key") == active_player:
                    rank_map[player.get("player_key")] = {"rank": int(player.get("rank")), "pos": [i for i in player.get("positions") if i != "Util"][0]}


        # go through multi positional players and if they have a higher rank, replace the player
        for p_key, val in rank_map.items():
            for player in players:
                if len(player.get("positions")) > 2 and player.get("player_key") not in daily_lineup:
                    if int(player.get("rank")) > val.get("rank") and val.get("pos") in player.get("positions"):
                        player["positions"] = val.get("pos")
                        daily_lineup.remove(p_key)
                        daily_lineup.append(player.get("player_key"))

        # add the remaining players
        for pos, count in used_positions.items():
            if "IR" in player.get("positions") or nhl_to_yahoo_abbr.get(player.get("team_key")) \
                    not in active_teams or pos == "Util":
                continue

                if player.get("player_key") not in daily_lineup and pos in player.get("positions") and used_positions.get(pos) > 0:
                    daily_lineup.append(player.get("player_key"))
                    used_positions[pos] = used_positions.get(pos) - 1
                    player['positions'] = pos

        # set the utils
        for player in players:
            if "IR" in player.get("positions") or nhl_to_yahoo_abbr.get(player.get("team_key")) not in active_teams or used_positions.get("Util") <= 0 or player.get("player_key") in daily_lineup:
                continue

            daily_lineup.append(player.get("player_key"))
            used_positions['Util'] = used_positions.get('Util') - 1
            player['positions'] = 'Util'


        # bench everyone else
        for player in players:
            if player.get("player_key") not in daily_lineup:
                player['positions'] = "BN"

        player_xml_list = []
        key_position_map = {}
        for p in players:
            key_position_map[p.get("player_key")] = p.get("positions")
        ...
        for player in daily_lineup:
            ...
            """ % (player, key_position_map.get(player))
